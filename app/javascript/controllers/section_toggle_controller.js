...
export default class extends Controller {
  ...
  #summaryTracklist(rows) {
    const items = rows
      .map((r) => this.#rowValues(r))
      .filter((v) => v.title || v.position)
    if (items.length === 0) {
  ...
  #summaryFormats(rows) {
    ...
    const badges = items.flatMap((v) => {
      const qty =
        v.qty && parseInt(v.qty) > 1 ? `${parseInt(v.qty)}\u00d7 ` : ""
  ...
  #summaryGeneric(rows) {
    ...
    if (items.length === 0) {
      ...
      const html = items
        .map(
          (t) =>
            `<li class="py-1.5 text-sm text-woodsmoke-200">${this.#esc(t)}</li>`
        )
        .join("")
